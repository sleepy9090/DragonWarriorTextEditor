    ...
    public partial class FormDialog9 : Form {
        ...
        private void buttonUpdateText_Click(object sender, EventArgs e) {
            try {
                ...
                backend.updateROMText(0x28, textBoxRC13.Text, 0x80D9, 0); //ь held the Rainbow Drop toward the sky.~
                ...
                backend.updateROMText(0xD, textBoxRC18.Text, 0x81AD, 0); //<Good night>~
                backend.updateROMText(0x1A, textBoxRC19.Text, 0x81BB, 0); //<Okay.~Good-bye, traveler>
                backend.updateROMText(0x5C, textBoxRC20.Text, 0x81D6, 0); //<Welcome to the traveler゜s Inn.~Room and board is Ɠ GOLD per night.~Dost thou want a room?'~
                ...
                backend.updateROMText(0x30, textBoxRC25.Text, 0x8296, 0); //<I thank thee.~Won゜t thou buy one more bottle?'~
                backend.updateROMText(0x54, textBoxRC26.Text, 0x82C7, 0); //<Will thou buy some Fairy Water for 38 GOLD to keep the Dragonlord゜s minions away?'~
                ...
                backend.updateROMText(0x37, textBoxRC30.Text, 0x837F, 0); //<Here,take this key.~Dost thou wish to purchase more?'~
                backend.updateROMText(0x54, textBoxRC31.Text, 0x83B7, 0); //<Magic keys!~They will unlock any door.~Dost thou wish to purchase one for Ɠ GOLD?'~
                ...
                backend.updateROMText(0x43, textBoxRC37.Text, 0x8483, 0); //<Thou said the ϊ.~I will buy thy ϊ for Ɠ GOLD.~Is that all right?'~
                backend.updateROMText(0x19, textBoxRC38.Text, 0x84C7, 0); //<What art thou selling?'~
                backend.updateROMText(0x26, textBoxRC39.Text, 0x84E1, 0); //<I will be waiting for thy next visit>
                backend.updateROMText(0x20, textBoxRC40.Text, 0x8508, 0); //<Dost thou want anything else?'~
        ...
        private void readRomText() {
            try {
                ...
                textBoxRC13.Text = backend.getROMText(0x28, 0x80D9, 0);
                ...
                textBoxRC18.Text = backend.getROMText(0xD, 0x81AD, 0);
                textBoxRC19.Text = backend.getROMText(0x1A, 0x81BB, 0);
                textBoxRC20.Text = backend.getROMText(0x5C, 0x81D6, 0);
                ...
                textBoxRC25.Text = backend.getROMText(0x30, 0x8296, 0);
                textBoxRC26.Text = backend.getROMText(0x54, 0x82C7, 0);
                ...
                textBoxRC30.Text = backend.getROMText(0x37, 0x837F, 0);
                textBoxRC31.Text = backend.getROMText(0x54, 0x83B7, 0);
                ...
                textBoxRC37.Text = backend.getROMText(0x43, 0x8483, 0);
                textBoxRC38.Text = backend.getROMText(0x19, 0x84C7, 0);
                textBoxRC39.Text = backend.getROMText(0x26, 0x84E1, 0);
                textBoxRC40.Text = backend.getROMText(0x20, 0x8508, 0);
