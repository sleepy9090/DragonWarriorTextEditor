    ...
    public partial class FormDialog6 : Form
    {
        ...
        private void buttonUpdateText_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                backend.updateROMText(0x12, textBoxEE3.Text, 0xAAD1, 0); //@ discovers the +.
                backend.updateROMText(0x2D, textBoxEE4.Text, 0xAAE4, 0); //Feel the wind blowing from behind the throne.
                backend.updateROMText(0x21, textBoxEE5.Text, 0xAB12, 0); //There is nothing to take here, @.
                backend.updateROMText(0x1A, textBoxEE6.Text, 0xAB34, 0); //Of GOLD thou hast gained %
                backend.updateROMText(0x33, textBoxEE7.Text, 0xAB4F, 0); //Fortune smiles upon thee, @.~Thou hast found the +.
                backend.updateROMText(0x1B, textBoxEE8.Text, 0xAB83, 0); //Unfortunately, it is empty.
                backend.updateROMText(0x7B, textBoxEE9.Text, 0xAB9F, 0); //Heed my voice,~<@, for this is Gwaelin.~To reach the next level thou must raise thy Experienceϋ × by %.~My hope is with thee>
                backend.updateROMText(0x29, textBoxEE10.Text, 0xAC1C, 0); //<From where thou art now, my castle lies$
                backend.updateROMText(0x13, textBoxEE11.Text, 0xAC48, 0); //% to the north and$
                backend.updateROMText(0x13, textBoxEE12.Text, 0xAC5D, 0); //% to the south and$
                backend.updateROMText(0xE, textBoxEE13.Text, 0xAC72, 0); //% to the east>
                backend.updateROMText(0xE, textBoxEE14.Text, 0xAC83, 0); //% to the west>
                ...
                backend.updateROMText(0x22, textBoxEE17.Text, 0xACBD, 0); //The ё attacked before @ was ready.
                backend.updateROMText(0xA, textBoxEE18.Text, 0xACE0, 0); //@ attacks!
                backend.updateROMText(0x24, textBoxEE19.Text, 0xACEB, 0); //The ё’s Hitϋ have been reduced by %.
                ...
                backend.updateROMText(0x1E, textBoxEE28.Text, 0xAE19, 0); //Thy Experience~increases by %.
                backend.updateROMText(0x18, textBoxEE29.Text, 0xAE38, 0); //Thy GOLD~increases by %.
                ...
                backend.updateROMText(0x16, textBoxEE34.Text, 0xAF0F, 0); //@ started to run away.
                backend.updateROMText(0x19, textBoxEE35.Text, 0xAF26, 0); //But was blocked in front.
                backend.updateROMText(0x10, textBoxEE36.Text, 0xAF40, 0); //@ used the Herb.
                ...
                backend.updateROMText(0x18, textBoxEE39.Text, 0xAF75, 0); //Thy Hitś decreased by %.
