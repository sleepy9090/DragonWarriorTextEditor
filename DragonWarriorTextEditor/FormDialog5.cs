    ...
    public partial class FormDialog5 : Form
    {
        ...
        private void buttonUpdateText_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                backend.updateROMText(0x27, textBoxDD6.Text, 0xA44C, 0); //<Thou art brave indeed to rescue me, @>
                ...
                backend.updateROMText(0x63, textBoxDD11.Text, 0xA4DB, 0); //<Forever shall I be grateful for the gift of my daughter returned to her home, @.~Accept my thanks>
                backend.updateROMText(0x1F, textBoxDD12.Text, 0xA540, 0); //<Now, Gwaelin, come to my side>
                backend.updateROMText(0x4A, textBoxDD13.Text, 0xA561, 0); //Gwaelin then whispers:~<Wait a moment,please.~I would give a present to @>
                backend.updateROMText(0x1A, textBoxDD14.Text, 0xA5AD, 0); //<Please accept my love, @>
                backend.updateROMText(0x36, textBoxDD15.Text, 0xA5C9, 0); //<And I would like to have something of thine--a token>
                backend.updateROMText(0x16, textBoxDD16.Text, 0xA601, 0); //<Please give me thy +>
                backend.updateROMText(0x46, textBoxDD17.Text, 0xA618, 0); //<Even when we two are parted by great distances, I shall be with thee>
                backend.updateROMText(0xD, textBoxDD18.Text, 0xA660, 0); //<Farewell, @>
                backend.updateROMText(0x11, textBoxDD19.Text, 0xA66F, 0); //<I love thee, @>~
                backend.updateROMText(0x18, textBoxDD20.Text, 0xA681, 0); //<Dost thou love me, @?'~
                ...
                backend.updateROMText(0x31, textBoxDD22.Text, 0xA6EC, 0); //<I am greatly pleased that thou hast returned, @>
                ...
                backend.updateROMText(0x33, textBoxDD25.Text, 0xA7BD, 0); //<Goodbye now, @.~Take care and tempt not the Fates>
                ...
                backend.updateROMText(0x30, textBoxDD28.Text, 0xA82F, 0); //<Welcome, @.~I am the Dragonlord--King of Kings>
                ...
                backend.updateROMText(0x5E, textBoxDD34.Text, 0xA996, 0); //If thou would take the +, thou must now discard some other item.~Dost thou wish to have the +?
                backend.updateROMText(0x19, textBoxDD35.Text, 0xA9F5, 0); //Thou hast given up thy +.
                backend.updateROMText(0x15, textBoxDD36.Text, 0xAA0F, 0); //What shall thou drop?
                backend.updateROMText(0x18, textBoxDD37.Text, 0xAA25, 0); //Thou hast dropped thy +.
                backend.updateROMText(0x13, textBoxDD38.Text, 0xAA3E, 0); //And obtained the +.
                backend.updateROMText(0x29, textBoxDD39.Text, 0xAA52, 0); //That is much too important to throw away.
                backend.updateROMText(0x20, textBoxDD40.Text, 0xAA7C, 0); //@ searched the ground all about.
        ...
        private void FormDialog5_Load(object sender, EventArgs e)
        {
            readRomText();
